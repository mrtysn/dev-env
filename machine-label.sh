...
#   c1 = Office (mert-cypher-m3max)     c2 = Home (mrtysn-mbp-m2max)
...
case "$(scutil --get LocalHostName 2>/dev/null || hostname -s)" in
    mert-cypher-m3max) MACHINE_LABEL="c1"; MACHINE_NAME="C1 (Office)" ;;
    mrtysn-mbp-m2max)  MACHINE_LABEL="c2"; MACHINE_NAME="C2 (Home)" ;;
